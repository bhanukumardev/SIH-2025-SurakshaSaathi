...
# Resolve paths relative to this file so imports work when module is loaded from anywhere
_BASE_DIR = os.path.dirname(__file__)
intents = json.load(open(os.path.join(_BASE_DIR, 'intents.json')))
words = pickle.load(open(os.path.join(_BASE_DIR, 'model', 'words.pkl'), 'rb'))
classes = pickle.load(open(os.path.join(_BASE_DIR, 'model', 'classes.pkl'), 'rb'))
...
def ensure_model_loaded():
    """Attempt to load the Keras model now. Returns True if model is loaded."""
    global model, _model_load_attempted, _model_load_failed
    if model is None and not _model_load_attempted:
        try:
            _model_load_attempted = True
            from tensorflow.keras.models import load_model as _load_model
            model = _load_model(os.path.join(_BASE_DIR, 'model', 'chatbot_model.keras'))
            _model_load_failed = False
        except Exception:
            _model_load_failed = True
    return bool(model is not None and not _model_load_failed)
